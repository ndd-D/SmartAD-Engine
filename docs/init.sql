...
INSERT INTO `smartad_user` (`username`, `password`, `nickname`, `role`, `status`) VALUES
('admin', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5Eu', '系统管理员', 'admin', 1);
...
CREATE TABLE IF NOT EXISTS `smartad_command` (
  ...
  `status`       VARCHAR(32)   NOT NULL DEFAULT '待AI处理'
                   COMMENT '状态: 待AI处理/处理中/AI提问中/已生成策略/执行失败',
  `ai_question`  TEXT          COMMENT 'AI追问内容',
  `fail_reason`  VARCHAR(500)  COMMENT '失败原因',
  `strategy_id`  VARCHAR(32)   COMMENT '关联策略ID',
...
CREATE TABLE IF NOT EXISTS `smartad_crowd` (
  ...
  `prefer_tags` VARCHAR(512)          COMMENT '偏好标签(JSON数组)',
  `active_time` VARCHAR(64)           COMMENT '活跃时间段',
  `age_range`   VARCHAR(32)           COMMENT '年龄范围',
  `gender`      VARCHAR(16)           COMMENT '性别倾向',
  `city_level`  VARCHAR(32)           COMMENT '城市级别',
  `created_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
INSERT INTO `smartad_crowd` (`crowd_tag`, `description`, `user_count`, `prefer_tags`, `active_time`, `age_range`, `gender`, `city_level`) VALUES
('young_fashion', '18-28岁时尚女性，对服饰美妆敏感，转化率高', 5000000, '["美妆","护肤","时尚","服饰"]', '18:00-23:00', '18-28', 'female', '一线/新一线'),
('middle_income', '30-45岁中等收入家庭，偏品质消费，客单价高', 3000000, '["家居","家电","教育","汽车"]', '08:00-22:00', '30-45', 'all', '二线及以上'),
('senior_tech', '45岁以上科技爱好者，忠诚度高，ROI稳定', 1500000, '["数码","健康","理财","旅游"]', '06:00-20:00', '45+', 'male', '三线及以上'),
('student', '18-25岁在校学生，价格敏感，活跃度高', 8000000, '["数码","美妆","服饰","零食"]', '12:00-24:00', '18-25', 'all', '一二线'),
('enterprise', 'B端企业采购决策人，客单价极高', 500000, '["办公设备","企业服务","云计算","咨询"]', '09:00-18:00', '25-55', 'all', '一二线');
